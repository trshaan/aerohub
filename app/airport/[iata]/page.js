    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-16 md:py-20">
          <p className="text-orange-500 text-sm font-mono tracking-[0.3em] mb-6">AIRPORT</p>
          <div className="flex items-baseline gap-6 mb-4">
            <h1 className="text-7xl md:text-9xl font-bold tracking-tight text-orange-500 font-mono">{airport.iata}</h1>
            <span className="text-2xl md:text-3xl font-mono text-white/30">{airport.icao}</span>
          </div>
          <h2 className="text-2xl md:text-4xl font-bold mb-3">{airport.name}</h2>
          <p className="text-white/60 text-lg">{airport.city}, {airport.country}</p>
        </div>
      </section>
      ...
      {operatingAirlines.length > 0 && (
        <section className="max-w-6xl mx-auto px-6 py-16 border-b border-white/10">
          <p className="text-orange-500 text-sm font-mono tracking-[0.3em] mb-3">CARRIERS</p>
          <h3 className="text-3xl font-bold mb-8">Airlines operating here</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {operatingAirlines.map((al) => (
              <Link key={al.iata} href={`/airline/${al.iata}`} className="border border-white/10 rounded-lg p-4 hover:border-orange-500/50 hover:bg-orange-500/[0.03] transition">
                <div className="font-mono text-orange-500 font-bold text-lg mb-1">{al.iata}</div>
                <div className="text-sm text-white/80 leading-tight">{al.name}</div>
              </Link>
            ))}
          </div>
        </section>
      ...
      {destinations.length > 0 && (
        <section className="max-w-6xl mx-auto px-6 py-16">
          <p className="text-orange-500 text-sm font-mono tracking-[0.3em] mb-3">DESTINATIONS</p>
          <h3 className="text-3xl font-bold mb-8">Where you can fly from here</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {destinations.map((d) => (
              <Link key={d.iata} href={`/airport/${d.iata}`} className="border border-white/10 rounded-lg p-4 hover:border-orange-500/50 hover:bg-orange-500/[0.03] transition">
                <div className="font-mono text-orange-500 font-bold text-xl mb-1">{d.iata}</div>
                <div className="text-sm text-white/80 leading-tight">{d.city}, {d.country}</div>
              </Link>
            ))}
          </div>
        </section>
      ...
      <footer className="border-t border-white/10 py-8 text-center text-white/40 text-sm">
        <p className="font-mono tracking-wider">
          <Link href="/" className="hover:text-orange-500 transition">BACK TO AEROHUB</Link>
        </p>
      </footer>
    </main>
  );
...
function Fact({ label, value }) {
  return (
    <div className="bg-black p-6">
      <div className="text-xs text-white/40 font-mono tracking-widest uppercase mb-2">{label}</div>
